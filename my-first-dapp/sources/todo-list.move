module my_addr::todo_list {
  ...
  // #[view]
  // public fun view_task(account: address, task_id: u64): Task acquires TodoList {
  //   assert!(exists<TodoList>(account), E_NOT_INITIALIZED);
  //   let todo_list = borrow_global<TodoList>(account);
  //   assert!(table::contains(&todo_list.tasks, task_id), ETASK_DOESNT_EXIST);
